...
API_TOKEN = '6446963690:AAEVDps6iZXwmadIMd5pCwmSK0y7MmQ8YBk'
PAYMENT_TOKEN = '381764678:TEST:68074'
...
async def show_sushi(message: types.Message):
    ...
    conn.close()
    await message.answer('Каталог суши и роллов 🍣')
    if prod_sushi:
...
async def show_drinks(message: types.Message):
    ...
    conn.close()
    await message.answer('Каталог напитков 🥤')
    if prod_drinks:
...
async def show_sous(message: types.Message):
    ...
    conn.close()
    await message.answer('Каталог соусов 🥣')
    if prod_sous:
...
async def show_cart(message: types.Message):
    ...
    if cart_items:
        ...
        try:
            for item in range(len(cart_items)-1):
                print(f'Item: {item}')
                product_img, product_id, product_name, product_price, quantity = cart_items[item]
                total_price += product_price * quantity
            ...
            product_img, product_id, product_name, product_price, quantity = cart_items[-1]
            total_price += product_price * quantity
            ...
            await bot.send_photo(message.chat.id, photo=f'{product_img}',
                                 caption=f'{product_name} - {product_price} руб.', reply_markup=keyboard1)
